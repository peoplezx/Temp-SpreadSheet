...
credentials = ServiceAccountCredentials.from_json_keyfile_name('readTemp-4c749e4b66d7.json', scope) ## Temp-3914c1792c0b.json replace Your Json file
gc = gspread.authorize(credentials)

wks = gc.open("TempSheet").sheet1  ## Temp replace your Work sheet name
	...
	while True:
		...
		try:
			for values in wks.col_values(1):  ## loop count row
    				x = x + 1
			rowToAdd = [time, temperatures[0], temperatures[1]] ## your data want to send
			wks.resize(x)
